...
- FIT/GPX/TCX manual import — gate-free, platform-agnostic route *enrichment* (NOT logging ease — that's already covered by Layer 0). User exports an activity file from Garmin/Coros/etc; client-side parse attaches the route to the existing session. Lower priority now that native GPS capture (separate session) owns the route; keep as a fallback for activities recorded only on a watch. (wearable-ingestion-spec.md Addendum, Layer 2)
- ✅ Logging ease from Garmin — what the user actually wanted — is delivered by Layer 0 (OS health floor) with no gate: Garmin syncs workout summaries to Apple Health + Health Connect, adapter auto-logs them. Caveats: poll-on-open not push (rule 6), summary-level not per-set. The blocked Garmin direct API was only needed for route + tier-3 metrics, neither of which is the logging-ease goal. (wearable-ingestion-spec.md Addendum, Reframe)
...
- Health Connect route reads are consent-gated per-route (other apps' routes return ConsentRequired; background reads restricted regardless) — fetch routes in-foreground during import and freeze onto the Observation immediately, or risk never getting them. Verified 2026-07-01. (outdoor-integrations.md v0.2 OS-floor audit)
- Sub-discipline tag on ingested workouts — user-set, tier-1 self-report; OS enums lump aggressively (`.paddleSports` = kayak+SUP+canoe, single `SKIING` on Android, `.climbing` with no bouldering split, `.surfingSports` includes kitesurf). The adapter suggests from the enum, never determines. Design into the Phase 3 adapter interface from day one. (outdoor-integrations.md v0.2)
...
- ✅ Climbing app research — DONE (2026-07-01): `climbing-apps-research.md` (Kaya, Crux, Toplogger, Mountain Project, boards, Lattice/Crimpd; import paths verified live). **Follow-up decision now unblocked:** finalize the climbing surface — the converged ascent record + a granularity ladder (session → sends → per-climb → per-attempt), which revises the spec's session-only stance for indoor bouldering. Bless before the climbing logging build. (training-logging-spec.md "Climbing", climbing-apps-research.md ⚑1)
- Mountain Project import — the API is dead (2020; onX still declining requests in 2026), but the per-user CSV tick export is alive (verified live 2026-07-01, no auth). Small client-side parser pass alongside 8a.nu CSV + BoardLib CSV; route tagging via OpenBeta (CC0 GraphQL, no key) + `@openbeta/sandbag` (MIT) for grade conversion. Do NOT scrape the MP route DB (onX C&D'd OpenBeta, 2021). (climbing-apps-research.md)
- Exercise demo / description content — stretch goal on library entries. (training-logging-spec.md "Exercise library")

---

## Sport expansion / outdoor adapters (self-contained passes, slot after Phase 3 — sequencing in `outdoor-sports-master-plan.md`)

- Add-now sport labels (identity layer only, minutes each): walk, ruck, row, row-erg, canoe, SUP, sail, windsurf, kitesurf, ski touring, snowshoe, XC ski, skate, open-water swim, martial arts/BJJ, dance. Ruck adds `load_kg` (+ optional gear snapshot) — the one new field the 2026-07-01 audit produced; no OS layer carries load, so it's our field or nobody's. (outdoor-integrations.md v0.2)
- Conditions-freeze adapters — each thin and independent: surf = NDBC buoy + CO-OPS tide + Open-Meteo marine (`models=era5_ocean` for history past Oct 2021); ski = avalanche.org v2 (tier-3, freeze WITH issue/expiry timestamps) + SNOTEL AWDB REST; hike = Overpass trail names (descriptive User-Agent REQUIRED — generic UAs get 406 since Apr 2026) + NWS/Open-Meteo weather; whitewater = USGS gauges (v0.1). Partially blocked by the Spot/place primitive decision. (outdoor-integrations.md v0.2)
- Spot / saved-place primitive — four sports now want it (whitewater runs↔gauges, wing spots↔wind stations, surf breaks↔buoy+tide, ski zones↔SNOTEL+avalanche zone). Bless as a small entity: id + name + latlng + linked condition sources. (outdoor-integrations.md Q4, outdoor-sports-master-plan.md ⚑4)
- Wave-count provenance flag on surf sessions — hand-counted / watch-counted / unknown; auto-detected counts are tier-2 with fidelity = detection method. (outdoor-integrations.md v0.2)
- Ski touring sub-type — skin-up segments are gym-shaped aerobic work (vertical gain, duration, HR → stimulus ledger); descents stay pure logbook. (outdoor-integrations.md v0.2)
- Concept2 Logbook adapter — the one genuinely open specialty API found (free OAuth2, granular scopes, currently no rate limits, webhooks, stroke-level data; their verified-vs-manual flag maps 1:1 to our fidelity). First OAuth adapter (Expo AuthSession, no backend needed); build when erg users exist. (outdoor-integrations.md v0.2)
- Triathlon-as-composition — multisport FIT files: each leg its own session message, transitions as `sport=transition`; parent Observation (tier-2 composition) + child sessions. Parser extension, not a new format; newer Garmin layouts need timestamp-based record slicing. (outdoor-integrations.md v0.2)
- Racket match surface (tennis / padel / pickleball) — the one genuine new-surface pressure from the gap audit (opponent, score, W/L; no open data ecosystem to lean on). Deliberately deferred; practice-duration labels meanwhile. (outdoor-integrations.md v0.2)
...
- **Native GPS tracking + map — DIRECTION CHANGE (2026-06-30), ✅ BLESSED 2026-07-02.** Reversed the documented "the app is not building a GPS tracker, it ingests from one" principle; blessed by Dylan and written into training-logging-spec.md § Outdoor / GPS (⚠ dropped). Placement decided the same day: **Phase 3 fast-follow.** Not a north-star violation (own-run recording is descriptive + pull-based). It becomes the primary route source (Layer 1), demoting wearable import + Garmin to enrichment. Build effort is its own thing: background location, battery, map render. (wearable-ingestion-spec.md Addendum, training-logging-spec.md)
