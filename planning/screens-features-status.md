...
**Important:** the new brand kit's tokens (Archivo/Space Grotesk/Space Mono/DM Sans; Earth `#8A7049` / Sky `#5E84A6` / Water `#4C8E85` / Body `#C15A39`) are **NOT yet in `src/theme/tokens.ts`** — that file still holds an older placeholder system (sandstone/olive/clay/slate; BarlowCondensed/Inter/JetBrainsMono). `planning/brand-kit.md` is stale by declaration, and `planning/brand-kit-gorge-draft.md` is an *earlier* draft than the kit Dylan cited on 2026-07-11 (it lacks the fonts and all four element hexes). Token-migration mechanics: `planning/rework/brand-integration.md`.
