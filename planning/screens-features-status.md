...
5 bottom tabs: **Home, Training, Map, Nutrition, Groups.** Profile is NOT a tab — it's a persistent top-right avatar on every tab (tap-through from any name in Groups too); Settings is a root modal reached via a gear near the avatar. Reflect, Benchmarks, Templates & Sections, Gear Quiver, and the Ring 3b summoned coach are tap-in surfaces. Current shipped tab bar on `main` is still **Today, Training, Nutrition, Reflect** (4 tabs, no Map, no Groups) — see `app/(tabs)/_layout.tsx`.
