...
*Planning doc. Pulls the Cora / trainhybrid / Edge / Nick Bare intel into one place. Companion to `product-overview.md` and `correlation-engine-spec.md`. Updates the Cora notes that previously lived only in the spec.*
...
- **They'll copy the words, not the mechanism.** Anyone can put "outcome-measured" and "honest" on a landing page. The defense is that the mechanism (back expenditure out of the trend, fidelity as a candidate cause, z-score against personal baseline, no surface AI) is genuinely incompatible with their engagement economics — so they can borrow the language but not the build.
