...
2. **In-app phone tracking** *(new — this doc)*. For the user with **no wearable**, the phone is the only GPS device there is, so recording with it is not a spine violation — the work is genuinely needed, not duplicated. This is a first-class capture surface, **not** a reluctant fallback: it is the *primary* path for the (large) watchless audience, and it's table stakes (every running app has it). It produces the same `route` coordinate array and the same Session shape as an import, `source: { type: 'manual' }`. It is **pull-only**: the user explicitly starts and stops it, and it never pushes anything mid-run (see § The line, below). **Background *continuation* of a user-initiated recording is required, not banned** — a 4-hour paragliding flight or a river day means the phone goes in a pocket and the screen locks; the recording must survive that. What's banned is recording the user *didn't* start (autonomous / surveillance-style tracking), never a user-started session going to the background. *(Amendment 2026-07-10 — earlier drafts read "never runs in the background" without qualification, which would kill exactly the sports this product is for. This is the corrected reading; the "pull-only" principle stands, applied to session *initiation*.)*
