...
import {
  ...
  rowToConditionsSnapshot,
  type ConditionsSnapshotRow,
} from './serialize';
...
export async function listSnapshotsForSpot(
  ...
  const rows = await d.getAllAsync<ConditionsSnapshotRow>(
  ...
  return rows.map(rowToConditionsSnapshot);
}
...
export async function getSnapshotById(
  ...
  const d = db ?? (await getDb());
  const row = await d.getFirstAsync<ConditionsSnapshotRow>(
    `SELECT ${COLUMNS} FROM conditions_snapshots WHERE id = ?;`,
    [id]
  );
  return row ? rowToConditionsSnapshot(row) : null;
}
