/**
 * fetchSkyConditionsSnapshot tests — MOCKED fetch, no live network. The wrapper
...
import type { OpenMeteoResponse } from '@core/skyConditions';
import { fetchSkyConditionsSnapshot } from '@/lib/conditions/skyOpenMeteo';
...
describe('fetchSkyConditionsSnapshot', () => {
  it('fetches the built Open-Meteo URL and returns a normalized snapshot', async () => {
    ...
    const snap = await fetchSkyConditionsSnapshot(SPOT, at, { fetchImpl: asFetch(fetchImpl) });
  ...
  it('mints a uuid id per freeze', async () => {
    ...
    const at = new Date('2026-07-05T21:20:00Z');
    const a = await fetchSkyConditionsSnapshot(SPOT, at, deps);
    const b = await fetchSkyConditionsSnapshot(SPOT, at, deps);
  ...
  it('dateLocal is the civil day at the SPOT, from the response offset', async () => {
    ...
    const snap = await fetchSkyConditionsSnapshot(SPOT, new Date('2026-07-06T05:30:00Z'), {
  ...
  it('returns a typed null on a non-OK response — no snapshot is invented', async () => {
    const snap = await fetchSkyConditionsSnapshot(SPOT, undefined, {
